...
var doseVoxels = new int[slices][,];
  ...
  {
    XSize = size, YSize = size, ZSize = slices,
   ...
   Patient = new PatientData { Id = "PHANTOM", LastName = "Test", FirstName = "E2E" },
          ...
          {
       Id = "E2E_Plan", CourseId = "C1",
               TotalDoseGy = 50.0, NumberOfFractions = 25, PlanNormalization = 100.0
      },
 ...
 DvhCurves = new List<DvhCurveData>(),
  ...
  string dir = baseDir;
     for (int i = 0; i < 8; i++)
            {
        string candidate = Path.Combine(dir, "ESAPI_EQD2Viewer.Tests", "TestFixtures", "snapshot");
      ...
      string fixturesDir = Path.Combine(dir, "ESAPI_EQD2Viewer.Tests", "TestFixtures");
