    ...
    public class ImageRenderingService : IImageRenderingService
    {
            ...
            EQD2Settings eqd2Settings = null)
        {
            ...
            try
            {
                ...
                // === EQD2 INTEGRATION ===
                // When EQD2 is enabled, we convert the reference dose to EQD2 as well,
                // so that percentage-based isodose levels (107%, 95%, etc.) remain
                // meaningful relative to the biologically effective prescription.
                ...
                if (eqd2Active)
                {
                    referenceDoseGy = EQD2Calculator.ToEQD2(referenceDoseGy, eqd2Settings.NumberOfFractions, eqd2Settings.AlphaBeta);
                    EQD2Calculator.GetVoxelScalingFactors(eqd2Settings.NumberOfFractions, eqd2Settings.AlphaBeta,
                        out eqd2QuadFactor, out eqd2LinFactor);
                }
                ...
                targetBitmap.AddDirtyRect(new Int32Rect(0, 0, _width, _height));
                return $"[{modeLabel}] CT Z: {currentSlice} | Dose Z: {doseSlice} | Max: {maxDoseInSlice:F2} Gy | Ref: {referenceDoseGy:F2} Gy";
            }
            finally
            {
                targetBitmap.Unlock();
